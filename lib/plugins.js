...
function testPlugin(bot, plugin, callback) {

  debug('testPlugins', 'testing', plugin.name, 'plugins');
  try {
    var pluginTest = plugin.module(bot, 'test');
    if (pluginTest.err) {
...
module.exports = function loadPlugins(bot, dir, callback) {
  var pluginList = [];
  fs.readdir(dir, function(err, plugins) {
    if (err) {
      ...
      for (var i = 0; i < plugins.length; i++) {
        ...
        testPlugin(bot, pluginObj, function(err) {
          if (err) {
